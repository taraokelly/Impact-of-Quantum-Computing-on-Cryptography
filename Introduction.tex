...
%For encryption, ciphertext (C) is calculated from the plaintext (PT) or original
%message (M) as follows:
%\[C = M^E  mod  n\]

%For decryption, calculate the plaintext (PT) from the ciphertext (CT) or encrypted
%message as follows:
%\[M = C^d mod n\]
